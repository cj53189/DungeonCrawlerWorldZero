...
(function loadInventoryUiScrollPatchAfterGameScripts() {
  function loadShopHardExit() {
    if (document.querySelector('script[src="./src/safe-room-shop-hard-exit.js"]')) return;
    const hardExit = document.createElement("script");
    hardExit.src = "./src/safe-room-shop-hard-exit.js";
    document.head.appendChild(hardExit);
  }

  function loadShopRenderFix() {
    const existingRenderFix = document.querySelector('script[src="./src/safe-room-shop-render-fix.js"]');
    if (existingRenderFix) {
      if (window.__dcwSafeRoomShopRenderFixInstalled) loadShopHardExit();
      else existingRenderFix.addEventListener("load", loadShopHardExit, { once: true });
      return;
    }
    const renderFix = document.createElement("script");
    renderFix.src = "./src/safe-room-shop-render-fix.js";
    renderFix.addEventListener("load", loadShopHardExit, { once: true });
    document.head.appendChild(renderFix);
  }

  function loadSafeRoomShopCloseFix() {
    const existingCloseFix = document.querySelector('script[src="./src/safe-room-shop-close-fix.js"]');
    if (existingCloseFix) {
      if (window.__dcwSafeRoomShopCloseFixInstalled) loadShopRenderFix();
      else existingCloseFix.addEventListener("load", loadShopRenderFix, { once: true });
      return;
    }
    const closeFix = document.createElement("script");
    closeFix.src = "./src/safe-room-shop-close-fix.js";
    closeFix.addEventListener("load", loadShopRenderFix, { once: true });
    document.head.appendChild(closeFix);
  }

  function loadSafeRoomShop() {
    const existingShop = document.querySelector('script[src="./src/safe-room-shop.js"]');
    if (existingShop) {
      if (window.__dcwSafeRoomShopInstalled) loadSafeRoomShopCloseFix();
      else existingShop.addEventListener("load", loadSafeRoomShopCloseFix, { once: true });
      return;
    }
    const shop = document.createElement("script");
    shop.src = "./src/safe-room-shop.js";
    shop.addEventListener("load", loadSafeRoomShopCloseFix, { once: true });
