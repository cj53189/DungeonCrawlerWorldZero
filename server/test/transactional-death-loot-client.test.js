...
function createHarness() {
  ...
  const context = {
    ...
    captureLocalCrawlerNetworkState: () => {
      ...
      // Reproduce the legacy F10 behavior. If the guard does not pre-arm the flag,
      // merely capturing this state immediately destroys the local loot.
...
test('downed crawler keeps loot through repeated/throttled state captures until server corpse confirmation', () => {
  ...
  assert.equal(first.lootSnapshot.inventory[0].id, 'sword_1');
  assert.equal(player.coins, 73);
  assert.equal(player.inventory.length, 1);
  assert.equal(clearCount(), 0, 'capturing a death state must not commit local loot loss');

  // A real call can be throttled after capture. The next attempt must carry the
  // same preserved loot instead of observing an already-empty inventory.
  const retry = context.captureLocalCrawlerNetworkState();
  assert.equal(retry.lootSnapshot.coins, 73);
  assert.equal(retry.lootSnapshot.inventory[0].id, 'sword_1');
  assert.equal(player.coins, 73);
  assert.equal(clearCount(), 0);

  context.handleMultiplayerServerMessage({
    type: 'player_corpse_created',
    runId: 'run_test',
    currentFloor: 2,
    corpse: { deadPlayerId: 'other_player', floor: 2 }
  });
  assert.equal(clearCount(), 0, 'another crawler corpse cannot commit local loot loss');

  context.handleMultiplayerServerMessage({
    type: 'player_corpse_created',
    runId: 'old_run',
    currentFloor: 2,
    corpse: { deadPlayerId: 'local_player', floor: 2 }
  });
  assert.equal(clearCount(), 0, 'a stale run corpse cannot commit local loot loss');
  ...
  assert.equal(clearCount(), 1, 'the matching server corpse is the transaction commit point');
  ...
  assert.equal(clearCount(), 1, 'duplicate death/corpse broadcasts must not commit twice');
});
...
test('floor start resets the death-loot transaction for the next life', () => {
  ...
  assert.equal(nextDeath.lootSnapshot.inventory[0].id, 'new_item');
  assert.equal(player.coins, 25);
  assert.equal(clearCount(), 1, 'the next death also waits for server confirmation');
});
